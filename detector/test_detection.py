...
from models.models.detector.retina_engine import TritonIS
import cv2
...
class Factory(ABC):
    ...
    def do_detect(self, batch, user_data):
        product = self.get_subclass()
        boxes, probs, landmarks, mask_probs = product.run(batch, user_data)
        return boxes, probs, landmarks, mask_probs


class Product(ABC):
    @abstractmethod
    def run(self, batch, user_data) -> str:
        pass

class Retinaface_mnet_cov2(Factory):
    def __init__(self, retinaface_config):
        self.retinaface_config = retinaface_config
        self.concrete = _retinaface_mnet_cov2(self.retinaface_config)

    def get_subclass(self) -> Product:
        return self.concrete
    
    def __str__(self):
        return 'Retinaface mnet cov2'
...
class _retinaface_mnet_cov2(TritonIS):
    def __init__(self, retinaface_config):
        self.retinaface_config = retinaface_config
        super().__init__(self.retinaface_config)
        self.get_model_specs()

        self.output_names = ['face_rpn_cls_prob_reshape_stride32', 'face_rpn_bbox_pred_stride32', 'face_rpn_landmark_pred_stride32', 'face_rpn_type_prob_reshape_stride32', 'face_rpn_cls_prob_reshape_stride16', 'face_rpn_bbox_pred_stride16',
            'face_rpn_landmark_pred_stride16', 'face_rpn_type_prob_reshape_stride16', 'face_rpn_cls_prob_reshape_stride8', 'face_rpn_bbox_pred_stride8', 'face_rpn_landmark_pred_stride8', 'face_rpn_type_prob_reshape_stride8']
    ...
    def preprocess(self, img, max_size):
    ...
    def run(self, input_blob, user_data):
        responses = self.execute(input_blob, user_data)
        toc = time.time()
        # print('detector inference time: ', toc - tic)
        tic = time.time()
        boxes = []
        probs = []
        landmarks = []
        ...
        for i in range(1):
            for response in responses:
                output_names = ['face_rpn_cls_prob_reshape_stride32', 'face_rpn_bbox_pred_stride32', 'face_rpn_landmark_pred_stride32', 'face_rpn_type_prob_reshape_stride32', 'face_rpn_cls_prob_reshape_stride16', 'face_rpn_bbox_pred_stride16',
                    'face_rpn_landmark_pred_stride16', 'face_rpn_type_prob_reshape_stride16', 'face_rpn_cls_prob_reshape_stride8', 'face_rpn_bbox_pred_stride8', 'face_rpn_landmark_pred_stride8', 'face_rpn_type_prob_reshape_stride8']
                results = [response.as_numpy(output_name) for output_name in output_names]
                bboxes, landmarks = self.postprocess(results, self.retinaface_config['threshold'])
                boxes = bboxes[:, 0:4]
                probs = bboxes[:, 4]
                try:
                    mask_probs = bboxes[:, 5]
                    ...
                    pass
        toc = time.time()
        # print('post processing inference time: ', toc - tic)
        return boxes, probs, landmarks, mask_probs
...
user_data = UserData()
detector = Retinaface_mnet_cov2(retinaface_config)
boxes, probs, landmarks, mask_probs = detector.do_detect(
                preprocessed_image, user_data)